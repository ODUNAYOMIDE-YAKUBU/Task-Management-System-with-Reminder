...
def _schedule_reminder(cur, task_id, user_id, due_date_str, hours_before):
    """hours_before can be fractional (e.g. 1.5 = 1h 30m, from days+hours+minutes combo)."""
    from datetime import timedelta
    try:
        if not hours_before or float(hours_before) <= 0:
            return
        due = datetime.fromisoformat(due_date_str)
        # Use total seconds to preserve minutes precision
        total_seconds = float(hours_before) * 3600
        remind_at = due - timedelta(seconds=total_seconds)
